...
def plagiarism(in_dir):
    ...
    #args = parse_options()
    out_dir, block_size = None,None
    #print(in_dir, out_dir, block_size)
    if path.exists(in_dir):  # Check if specified path exists
        ...
        if len(listdir(in_dir)) > 1:  # Check if there are at least 2 files at specified path
            ...
            if wait_for_file(results_directory, 60): 
                ...
                return difflib_scores# Open results HTML table
...
#print(plagiarism(r'C:\Users\satwi\Downloads\Essay_grade\test flask\test_flask\plagiarism'))
